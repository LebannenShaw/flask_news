...
def add_news():
    ...
    if form.validate_on_submit():
        ...
        duplicateNews = News.query.filter(News.title == news.title).first()
        if duplicateNews:
            flash('Новость с таким заголовком уже существует!', 'alert-danger')
...
def add_category():
    ...
    if form.validate_on_submit():
        ...
        duplicateCategory = Category.query.filter(Category.title == category.title).first()
        if duplicateCategory:
            flash('Категория с таким именем уже существует!', 'alert-danger')
...
@app.route("/category_detail/<string:name>")
